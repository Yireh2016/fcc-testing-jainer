...
suite('Functional Tests', function() {
  ...
  suite('Integration tests with chai-http', function() {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function(){
      ...
      test('Test GET /hello with your name',  function(done){ // Don't forget the callback...
         chai.request(server)             // 'server' is the Express App
          .get('/hello?name=biza') /** <=== Put your name in the query **/ 
          .end(function(err, res){        // res is the response object
            ...
            assert.equal(res.status, 200);
             assert.equal(res.text, 'hello biza'/** <==  Put your name here **/);
            done();   // Always call the 'done()' callback when finished.
          });
      });
    ...
    suite('PUT /travellers', function(){
      ...
      test('send {surname: "Colombo"}',  function(done){
       ...
       chai.request(server)
        ...
        .end(function(err, res){
          ...
          assert.equal(res.type,'application/json', 'res.type must be app/json'); // remove this after adding tests+
          assert.equal(res.body.name,'Cristoforo', 'body name:Cristoforo');
          assert.equal(res.body.surname,'Colombo', 'body  surname:Colombo');
      ...
      test('send {surname: "da Verrazzano"}', function(done) {
        ...
        .end((err,res)=>{
          assert.equal(res.status,200, 'status 200');
          assert.equal(res.type,'application/json', 'res.type must be app/json'); // remove this after adding tests+
          assert.equal(res.body.name,'Giovanni', 'body name:Giovanni');
          assert.equal(res.body.surname,'da Verrazzano', 'body  surname:da Verrazzano');
          done();
        })
  ...
  Browser.site = 'https://fcc-testing-jainer.glitch.me'; 
